...
export function TestCase({ documentId }: { documentId?: any }) {
  ...
  const handleSave = async () => {
    try {
      ...
      const category = selectConvert(values.category[1]);
      const types = selectConvert(values.types[1]);
      const steps = values.steps[1];
      const result = values.result[1];
      const division = values.division[1];
      console.log(
        await axios.post(
          `${process.env.NEXT_PUBLIC_HOST}:${process.env.NEXT_PUBLIC_PORT}/api/document/testcase/${documentId}`,
          {
            category,
            types,
            steps,
