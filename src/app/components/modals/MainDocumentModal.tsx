...
export function MainDocumentModal({
  open,
  confirmLoading,
  onCancel,
}: ModalProps) {
  ...
  const handleOk = async function () {
    try {
      ...
      const data = {
        ...
        title: values.title,
        intro: values.intro,
        role: values.roles.slice(1),
        employeeId: values.names.slice(1),
        jobPositionId: values.employee.slice(1),
      };
      console.log(await axios.post("/api/document", data));
    } catch (error) {
      console.log(error);
    }
  };
