...
export function ConfirmEditDocument({ open, onCancel }: ModalProps) {
  const [caseForm] = Form.useForm();
  const [getEmployee, setEmployee] = useState<any>([]);
  const [search, setSearch] = useState("");
  const { documentId, documentName } = globalState();
  ...
  return (
    ...
    >
      {contextHolder}
      <div className="">
        <p className="font-bold text-2xl">Баталгаажуулах хуудас</p>
      </div>
