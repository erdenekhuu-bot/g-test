    ...
    if (!employee) {
      ...
      try {
        const birthday = new Date(employee.birthDate)
          .toISOString()
          .split("T")[0];
        const databirthday = new Date(data.birthDate)
          .toISOString()
          .split("T")[0];
        const created = new Date(employee.timeCreated)
          .toISOString()
          .split("T")[0];
        const dataCreated = new Date(data.timeCreated)
          .toISOString()
          .split("T")[0];
        const updated = new Date(employee.timeUpdated)
          .toISOString()
          .split("T")[0];
        const dataUpdated = new Date(data.timeUpdated)
          .toISOString()
          .split("T")[0];
